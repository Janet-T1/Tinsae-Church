...
const MemberForm = () => {
    ...
    const onSubmit = async (data) => {
        try {
            const response = await fetch("https://formspree.io/f/xnnpqjbz", {
                method: "POST",
        ...
        <div className="w-screen h-screen flex items-center justify-center bg-[#2b2b2b]">
            <div className="w-full md:w-3/4 lg:w-2/3 bg-[#121211] p-12 mt-10">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    ...
                    <input {...register("birthday", { required: "Birthday is required" })} type="date" placeholder="Birthday" className="p-2 border bg-gray-100 text-black input-placeholder" />
                    ...
                    <button type="submit" className="p-2 bg-red-700 text-white font-semibold rounded hover:bg-red-800 transition-colors text-xl">Join</button>
                </form>
            </div>
        </div>
    );
